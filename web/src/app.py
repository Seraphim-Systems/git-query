...
WEBFRONTEND_DIR = os.path.join(os.path.dirname(__file__), "..", "public")
...
def _proxy_to_gateway(gateway_path, method=None, forward_cookies=True):
    ...
    flask_resp = make_response(
        jsonify(body) if isinstance(body, (dict, list)) else body, resp.status_code
    )
...
def health():
    ...
    return jsonify({"status": "healthy", "service": "webserver"}), 200
