...
from src.recommender.training.trainers.reranker_cross_encoder_trainer import (
    RerankerCrossEncoderTrainer as RerankerTrainer,
)
    ...
    with patch("src.recommender.training.trainers.reranker_cross_encoder_trainer.settings") as mock_settings:
        ...
        with patch("src.recommender.training.trainers.reranker_cross_encoder_trainer.CrossEncoder") as MockCrossEncoder:
            ...
            with patch("src.recommender.services.registry_service.ModelRegistryService") as MockRegistry:
                ...
                with patch(
                    "src.recommender.training.trainers.reranker_cross_encoder_trainer.os.makedirs"
                ) as mock_makedirs:
                    with patch(
                        "src.recommender.training.trainers.reranker_cross_encoder_trainer.os.remove"
                    ) as mock_remove:
                        with patch(
                            "src.recommender.training.trainers.reranker_cross_encoder_trainer.os.path.isdir"
                        ) as mock_isdir:
                            mock_isdir.return_value = True  # Assume directories

                            with patch(
                                "src.recommender.training.trainers.reranker_cross_encoder_trainer.shutil.rmtree"
                            ) as mock_rmtree:
                                with patch(
