...
check_service "MongoDB" "docker exec git-query-mongodb mongosh --quiet --eval 'db.adminCommand(\"ping\")'"
echo ""
...
check_service "Redis" "docker exec git-query-redis redis-cli ping | grep -q PONG"
echo ""

# Check Cosmos DB Emulator (optional)
echo -e "${CYAN}Checking Cosmos DB Emulator (optional)...${NC}"
if docker ps --filter "name=git-query-cosmos-db" | grep -q "Up"; then
...
echo "  Start dev:          docker compose -f infrastructure/docker/docker-compose.yml -f infrastructure/docker/docker-compose.dev.yml up -d"
echo "  Start prod:         docker compose -f infrastructure/docker/docker-compose.yml -f infrastructure/docker/docker-compose.prod.yml up -d"
echo ""
