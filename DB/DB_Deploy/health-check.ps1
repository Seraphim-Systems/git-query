...
Test-Service "MongoDB" {
    docker exec gitquery-mongodb mongosh --quiet --eval "db.adminCommand('ping')" 2>&1 | Out-Null
}
Write-Host ""
...
Test-Service "Redis" {
    $result = docker exec gitquery-redis redis-cli ping 2>&1
    $result -match "PONG"
}
Write-Host ""
...
Test-Service "PostgreSQL" {
    docker exec gitquery-postgres pg_isready -U user 2>&1 | Out-Null
}
Write-Host ""
...
try {
    $cosmosResponse = docker ps --filter "name=gitquery-cosmos-db" --format "{{.Status}}"
